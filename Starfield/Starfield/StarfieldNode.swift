...
class ParallaxStarfieldNode : SKShapeNode {
	...
	func addStarfield(starCount: Int, color: UIColor, speed: CGFloat) {
		var starfield = Starfield(width: size.width, height: size.height, starCount: starCount, color: color, speed: speed)
		var starfieldNode = StarfieldNode(size: size, starfield: starfield)
...
class Starfield {
	...
	func getPath() -> CGMutablePathRef {
		...
		for star in stars {
			CGPathAddRect(path, nil, CGRectMake(star.x, star.y, 0.5, 0.5))
		}
